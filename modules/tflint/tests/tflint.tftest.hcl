# Regression coverage for the generated tflint workflow.
#
...
run "renders_no_trailing_whitespace" {
  ...
  assert {
    condition     = yamldecode(output.workflow).jobs.tflint.strategy.matrix.path == ["./", "./modules/first"]
...
run "job_name_is_distinct" {
  ...
  assert {
    condition     = can(yamldecode(output.workflow).jobs.tflint)
    error_message = "The job must be named tflint, so its status context says what actually ran."
  }

  assert {
    condition     = !can(yamldecode(output.workflow).jobs["terraform-validate"])
    error_message = "tflint must not claim the terraform-validate name; it does not run terraform validation, and two workflows using it produced byte-identical contexts."
  }
}
...
run "paths_default_to_the_repository_root" {
  ...
  assert {
    condition     = yamldecode(output.workflow).jobs.tflint.strategy.matrix.path == ["./"]
