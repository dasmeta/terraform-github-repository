resource "github_repository_file" "githooks-default-files" {
  ...
  content             = templatefile("${path.module}/${each.value.local_path}", { paths = var.paths, aws-region = var.aws-region, aws-access-key-id = var.aws-access-key-id, aws-secret-access-key = var.aws-secret-access-key, path = var.path })
